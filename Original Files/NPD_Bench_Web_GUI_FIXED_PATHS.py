...
def process_input():
    ...
    if test == 1:
        path = os.path.join(
            r"C:\Users", f"{first}.{last}", "Desktop", "Fake Sharepoint",
            "LON-10095.1-Macallan - Documents", "03 - Technical", "06 - Test & Demonstrations",
            "06 Flight Phase Test Data", "PMA Tile", "NPDoverTemp"
        )

    # TEST 2 → Single Tile Bench NPD
    elif test == 2:
        path = os.path.join(
            r"C:\Users", f"{first}.{last}", "Redwire Space",
            "LON-10095.1-Macallan - Documents", "03 - Technical", "06 - Test & Demonstrations",
            "06 Flight Phase Test Data", "PMA Tile"
        )
    ...
    # TEST 3 → Full PMA Array Bench NPD
        ...
        path = os.path.join(
            r"C:\Users", f"{first}.{last}", "Desktop", "Fake Sharepoint",
            "LON-10095.1-Macallan - Documents", "03 - Technical", "06 - Test & Demonstrations",
            "06 Flight Phase Test Data", "PMA ARRAY", "Test Hat"
        )
    ...
    # Save credentials
    user = f"{first}.{last}@redwirespace.com"
...
tk.Button(root, text="OverTemp NPD", command=lambda: set_test(2)).grid(
...
tk.Button(root, text="Full PMA Array Bench NPD", command=lambda: set_test(3)).grid(
    row=3, column=0, columnspan=2, pady=25
)
