...
def generate_plots(params):
    ...
    folderA = get_RUN_A()  # e.g. "Run_1_8.953A"
    folderB = get_RUN_B()  # e.g. "Run_2_9.362A"

    lmoFolderA = os.path.join(full_sn_path, folderA)
    lmoFolderB = os.path.join(full_sn_path, folderB)
    ...
    print("\n=== Files Found in Run B ===")
    for f in filesNPDB + filesSparB:
        print(" -", f, "\n")
    ...
    def plotNPD(filesA, filesB):
        ...
        plt.plot(ref_freq_win, lower, color='red', alpha=1, marker='o', markersize=5, markevery=100,
                 label='Lower bound')
        plt.plot(ref_freq_win, upper, color='red', alpha=1, marker='x', markersize=5, markevery=100,
                 label='Upper bound')
    ...
    def plotS21(filesA, filesB):
        ...
        for file in filesA + filesB:
            ...
            # ---- Apply Calibration (ADD positive loss) ----
            ...
            # Save windowed points for PASS/FAIL
            s21_window = s21_corr[700:2100]
        ...
        # Plot bounds
        plt.plot(freq_ghz[700:2100], lower_bound, 'ro-', markersize=5, markevery=100, label='Lower bound')
        plt.plot(freq_ghz[700:2100], upper_bound, 'rx-', markersize=5, markevery=100, label='Upper bound')
        ...
        title = f'Before and After: Test Hat S21 (Calibrated), {status}'
        plt.title(title)
        plt.xlabel('Frequency (GHz)')
        plt.ylabel('S21 (dB)')
